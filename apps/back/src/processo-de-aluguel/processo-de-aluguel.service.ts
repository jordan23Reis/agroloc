...
export class ProcessoDeAluguelService {
  ...
  async findProcessoDeAluguel(idProcessoDeAluguel: string) {
    const processoDeAluguel = await this.processoDeAluguelModel.findById(
      idProcessoDeAluguel
    );
  ...
  async findProcessoDeAluguelFinalizadosDeUsuario(idUsuario: string) {
    const processosDeAluguelFinalizados =
      await this.processoDeAluguelModel.find({
        Status: {
          $in: ['A Avaliar', 'Avaliado'],
        },
  ...
  async create(idMaquina: string, idLocador: string, idLocatario) {
    ...
    const tipoPreco = await this.tipoPrecoService.findOne(
      maquina.Preco.Tipo.idTipo
    );
    const informacoesBancariasLocador =
      await this.usersService.findInformacoesBancarias(idLocador);
    ...
    const createdProcessoDeAluguel =
      this.processoDeAluguelModel.create(ProcessoDeAluguel);
    return createdProcessoDeAluguel;
  }

  async findExistingProcessoDeAluguelAAceitar(
    idMaquina: string,
    idLocador: string,
    idLocatario: string
  ) {
  ...
  async aceitarProcessoDeAluguel(idProcessoDeAluguel: string) {
    ...
    const maquinaAtreladaAprocesso = await this.maquinaService.findOne(
      processoDeAluguel.Maquina.idMaquina.toString()
    );
  ...
  async selecionarPreco(idProcessoDeAluguel, pagamentoDto: PagamentoDto) {
    ...
    processoDeAluguel.Pagamento.QuantificadorPreco =
      pagamentoDto.QuantificadorPreco;
    processoDeAluguel.Pagamento.Valor =
      pagamentoDto.QuantificadorPreco *
      processoDeAluguel.Pagamento.Preco.ValorPorTipo;
