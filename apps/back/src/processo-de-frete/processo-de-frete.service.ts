...
export class ProcessoDeFreteService {
  ...
  async findProcessoDeFrete(idProcessoDeFrete: string) {
    const processoDeFrete = await this.processoDeFreteModel.findById(
      idProcessoDeFrete
    );
  ...
  async create(
    ...
    const informacoesBancariasFreteiro =
      await this.usersService.findInformacoesBancarias(idFreteiro);
    const enderecoSolicitante = solicitante.CadastroComum.Enderecos.find(
      (end) => end._id.toString() == enderecoSolicitanteSelecionado
    );
    const maquinaSolicitante = await this.maquinaService.findOne(idMaquina);
    const processoDeAluguel = await this.processoDeAluguelService.findOne(
      idProcessoDeAluguel
    );
    ...
    const createdProcessoDeFrete = await this.processoDeFreteModel.create(
      ProcessoDeFrete
    );
  ...
  async aceitarProcessoDeFrete(idProcessoDeFrete: string, idVeiculo: string) {
    ...
    const usuarioFreteiroAtreladoAoProcesso = await this.usersService.findOne(
      processoDeFrete.Envolvidos.Freteiro.idFreteiro.toString()
    );
  ...
  async recusarProcessoDeFrete(idProcessoDeFrete: string) {
    ...
    const processoDeAluguel = await this.processoDeAluguelService.findOne(
      processoDeFrete.idProcessoDeAluguel.toString()
    );
