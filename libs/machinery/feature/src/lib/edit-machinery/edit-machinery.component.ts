...
@Component({
  selector: 'agroloc-edit-machinery',
  templateUrl: './edit-machinery.component.html',
  styleUrls: ['./edit-machinery.component.scss'],
})
export class EditMachineryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.accountService.editedMachinery$.subscribe((response) => {
      this.itemSelectedForEdit = response;
    });
  }
      ...
      .subscribe(([account, profile]) => {
        if (
          ...
          this.machineryService
            .updateMachinery(this.itemSelectedForEdit, machineryData)
            .subscribe((response) => {
              this.snackBar.open(
